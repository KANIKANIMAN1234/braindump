...
## 8.3 マルチテナント関連（2026-05-30）

| No | 発生箇所 | 条件/エラー | 対処 |
|---|---|---|---|
| C-01 | タスク一覧 | `members.status` が `invited` | `active` に更新 |
| C-02 | タスク一覧 | `line_user_id` に改行・空白 | trim して LIFF の userId と一致させる |
| C-03 | タスク一覧 | `organization_id` 未設定の旧タスク | `phase2_backfill_task_org.sql` |
| C-04 | 代表登録 | DB 手入力で activate 未実施 | 招待 URL から再登録 |
| C-05 | 組織 | 段数を変えたい | 未対応（初回のみ確定） |

詳細: [docs/11_Phase2_追補_2026-05-30.md](docs/11_Phase2_追補_2026-05-30.md)
...
## 11. Phase 1 / 2（B2B マルチテナント）

- 運営コンソール: `/app/platform-console.html`
- 組織階層: 0（代表のみ）/ 1 / 2 / 3 段。初回ウィザードのみ（事後変更なし）
- データスコープ: `lib/data-scope.js`（代表は法人内全タスク）
- 仕様書: [docs/09_Phase1_マルチテナント.md](docs/09_Phase1_マルチテナント.md) / [docs/10_Phase2_組織階層.md](docs/10_Phase2_組織階層.md) / [docs/11_Phase2_追補_2026-05-30.md](docs/11_Phase2_追補_2026-05-30.md)
