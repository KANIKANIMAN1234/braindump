...
ユーザーはチャットUIで **タスク管理**・**日々の気づき記録**・**プロンプト保存**・**朝ブリーフィング**・**今日の振り返り** を行い、音声入力にも対応する。
...
| `qa-export` | 📤 気づき/振り返りエクスポート | 気づき・今日の振り返りを同じDropboxフォルダへCSV出力 |
| `qa-briefing` | 🌅 朝ブリーフィング | 未完了タスクを要約して `briefing_logs` に保存 |
| `qa-reflection` | 📝 今日の振り返り | 本文→カテゴリ選択→ `daily_reflections` に日付単位で保存（同日は上書き） |
...
| 今日の振り返り | 本文＋タグで記録し `daily_reflections` に日付単位で保存（同日は上書き）。毎日の振り返り習慣化を目的とした項目。気づきと同じDropboxフォルダへCSVエクスポート可能 |
...
`tasks` / `insights` / `prompts` / `briefing` / `briefings` / `reflection` / `reflections` は該当ツール実行時のみ付与。
